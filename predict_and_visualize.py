...
from datapreprocess import ISSDockingDataset, get_dataloaders

# --- 1. Configuration ---
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
MODEL_PATH = "resnet50_docking_best.pth"
...
def predict_on_test_set(model):
    test_dataset = ISSDockingDataset(csv_path=TEST_CSV, img_dir=IMG_DIR, is_train=False)
...
def save_predictions_to_csv(predictions, actuals, image_ids):
    ...
    print(f"\n✓ Saved detailed predictions: {csv_path}")
...
if __name__ == "__main__":
    ...
    predictions, actuals, image_ids, test_dataset = predict_on_test_set(model)
    ...
    plot_metrics_summary(predictions, actuals)
    save_predictions_to_csv(predictions, actuals, image_ids)
